...
services.factory('beerService', function ($http, $q) {
    ...
    return {
        ...
        //To Do: This function requires getBeerList to have already run.
        //If this ever gets used in a browser you will not be able to 
        //navigate directly to a beer URL.
        getBeerById: function (beerId) {
            var deferred = $q.defer(),
                beerLen = beers.length;

            for (var i = 0; i < beerLen; i++) {
                if (beers[i].beer_id == beerId) {
                    deferred.resolve(beers[i]);
                    break;
                }
            }
